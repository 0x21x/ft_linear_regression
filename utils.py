...
def get_model_parameters(intercept: Optional[float] = 0, slope: Optional[float] = 0,
    ...
    try:
        ...
        logging.warning('The model has not been trained yet.')
...
def parse_and_check_csv(normalized:bool=True) -> pd.DataFrame:
    try:
        ...
        if data.columns.size != 2:
            logging.error('The CSV file must have exactly 2 columns.')
            raise SystemExit
        if data.columns[0] != 'km' or data.columns[1] != 'price':
            logging.error('The columns must be named "km" and "price".')
            raise SystemExit
        ...
        logging.error('The CSV file is not properly formatted.')
        raise SystemExit

    for column in data.columns:
        if data[column].isnull().values.any():
            logging.error('The CSV file must not contain any NaN values.')
            raise SystemExit
        if not data[column].dtype == 'float64' and not data[column].dtype == 'int64':
            logging.error('The columns must contain only numeric values.')
            raise SystemExit
